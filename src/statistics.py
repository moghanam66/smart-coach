...
class Statistics:
    def __init__(self, court_tracker: CourtDetector, players_detection: DetectionModel):
        ...
        self.bottomdistarr = None
        self.topdistarr = None
    ...
    def players_distances(self):

        court_width = self.court_tracker.court_reference.court_total_width
        court_height = self.court_tracker.court_reference.court_total_height
        dits_list = []
        print(self.feet_bottom.shape[0])
        for i in range(self.feet_bottom.shape[0] - 1):
            if i == 0 :
                dits_list.append(0)
            else:

                distance = math.dist([int(self.feet_bottom[i , 0]) , int(self.feet_bottom[i , 1])] ,
                [ int(self.feet_bottom[i-1 , 0]) ,  int(self.feet_bottom[i-1 , 1]) ])
                if distance < 40:
                    dits_list.append(dits_list[-1] + distance)
                else:
                    dits_list.append(dits_list[-1])

        scale =  10.97 / court_width
        for i in range(len(dits_list)):
            dits_list[i] = dits_list[i]*scale 

        self.bottomdistarr = dits_list

    def player2_distances(self):

        court_width = self.court_tracker.court_reference.court_total_width
        court_height = self.court_tracker.court_reference.court_total_height
        dits_list = []
        print(self.feet_top.shape[0])
        for i in range(self.feet_top.shape[0] - 1):
            if i == 0 :
                dits_list.append(0)
            else:

                distance = math.dist([int(self.feet_top[i , 0]) , int(self.feet_top[i , 1])] ,
                [ int(self.feet_top[i-1 , 0]) ,  int(self.feet_top[i-1 , 1]) ])
                if distance<40:
                    dits_list.append(dits_list[-1] + distance)
                else:
                    dits_list.append(dits_list[-1])
