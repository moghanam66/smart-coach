...
class PoseExtractor:
    def __init__(self, person_num=1, box=False, dtype=torch.FloatTensor):
        ...
        """
        self.pose_model = torchvision.models.detection.keypointrcnn_resnet50_fpn(weights=KeypointRCNN_ResNet50_FPN_Weights.DEFAULT)
    ...
    def extract_pose_p1(self, image, player_boxes ,court_detector,frame_i):
        """
        ...
        # else:
        margin = 0
        patch = image[int(court_detector.net[1])-50: ,: ]
        a3=np.array( [[[0,0],[600,0],[0,400]]], dtype=np.int32 )
        a2=np.array( [[[width,0],[width-600,0],[width,400]]], dtype=np.int32 )
        cv2.fillPoly(patch,a3,color=(0,0,0))
        cv2.fillPoly(patch,a2,color=(0,0,0))
        xt, yt, xb, yb = 0, int(court_detector.net[1])-50, width, height
            #patch = image.copy()
        ...
        for keypoints, keypoint_scores, score in zip(p[0]['keypoints'][:self.person_num], p[0]['keypoints_scores'],
            ...
            if score > self.SCORE_MIN:
                ...
                self.data_p1.append(x_data + y_data)
            
        if len(self.data_p1) < frame_i:
            self.data_p1.append(dummy_data)
        stickman[max(yt - margin, 0):min(yb + margin, height), max(xt - margin, 0):min(xb + margin, width)] = patch_zeros

        return stickman

    def extract_pose_p2(self, image, player2_box,frame_i):
        """
        extract pose from given image using pose_model
        :param player_boxes:
        :param image: ndarray, the image we would like to extract the pose from
        ...
        for keypoints, keypoint_scores, score in zip(p[0]['keypoints'][:self.person_num], p[0]['keypoints_scores'],
            ...
            if score > self.SCORE_MIN:
                ...
                # create the stickman using the keypoints we found
                self._add_lines(patch_zeros, keypoints, keypoint_scores)
            self.data_p2.append(x_data + y_data)
        if len(self.data_p2) < frame_i:
            self.data_p2.append(dummy_data)
        stickman[max(yt - margin, 0):min(yb + margin, height), max(xt - margin, 0):min(xb + margin, width)] = patch_zeros

        return stickman    

    def save_to_csv(self, output_folder,p1=True):
        """
        ...
        if p1:
            df = pd.DataFrame(self.data_p1, columns=columns_x + columns_y)
            outfile_path = os.path.join(output_folder, 'stickman_data_p1.csv')
        else:
            df = pd.DataFrame(self.data_p2, columns=columns_x + columns_y)
            outfile_path = os.path.join(output_folder, 'stickman_data_p2.csv')
